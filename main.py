...
def run():
    print("Prüfe neue Discovery Deals...")
    ...
    for deal in deals:
        ...
        if not aircall_id:
            print(f"⚠️ Kein Aircall Call gefunden für {deal_name}")
            continue
        ...
        if not transcript:
            print(f"⚠️ Kein Transkript verfügbar für {deal_name}")
            continue
        ...
        print(f"✅ Analyse abgeschlossen für {deal_name}")
